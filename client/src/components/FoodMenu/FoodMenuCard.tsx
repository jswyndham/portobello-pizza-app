	...
	return (
		<section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 2xl:gap-14 justify-center items-center">
			{foodItems.map((food) => (
				<article
					key={food._id}
					className="w-80 h-96 border border-slate-400 rounded-lg p-2 bg-slate-200 bg-opacity-70"
				>
					<div className="w-full h-full flex flex-col">
						{food.imageUrl ? (
							<img
								...
								alt={food.name}
								className="h-4/5 w-full object-cover border border-slate-300 rounded-md"
							/>
						) : (
							<img
								src={food.imageUrl || '/placeholder.jpg'}
								alt="No image available"
								className="h-4/5 w-full object-cover flex border border-slate-300 bg-white items-center justify-center rounded-md text-xl font-cinzel"
							/>
						)}
						...
						<div className="flex flex-col justify-between items-center p-3">
							...
							<div className="h-14 flex items-center">
								<p className="text-sm text-gray-600 text-center flex-grow font-sans font-bold">
									{food.ingredients.join(', ')}
								</p>
							</div>
							<p className="text-lg mt-2 flex-grow font-cinzel">
								฿ {food.price}
							</p>
						</div>
					</div>
