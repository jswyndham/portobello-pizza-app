...
function SidesMenu() {
		...
		const fetchMenuItems = async () => {
			try {
				...
				const response = await fetch(
					'http://localhost:5001/api/v1/foodMenu?menuCategory=SIDE'
				);
	...
	if (menuItems.length === 0) {
		return <ItemNotFound item="side menu items" />;
	}
		...
		<>
			...
			<article className="w-screen h-full bg-main-gradient min-h-screen">
				<Helmet>
					...
					<meta
						name="description"
						content="Enjoy the best Italian side dishes on Koh Tao Island at Porto Bello, Koh Tao. Check out our variety of side dishes with fresh ingredients."
					/>
					<link
						rel="canonical"
						href="http://www.portobello.com/sidesmenu"
					/>
				</Helmet>
				<MenuJSONLD menuItems={menuItems} />
				<article className="h-full pt-12 px-2 xl:pt-24 flex flex-col justify-center items-center">
					<HeadingOne headingOneText={'Sides Menu'} />
